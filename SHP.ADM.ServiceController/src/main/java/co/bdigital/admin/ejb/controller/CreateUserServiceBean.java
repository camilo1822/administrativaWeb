...
@LocalBean
public class CreateUserServiceBean extends CallServiceUtil implements
        ServiceController {
    ...
    public ResponseMessageObjectType executeOperation(
        ...
        if (!this.sch.validateRequest(request.getRequestMessage()
                .getRequestBody().getAny(), new CreateUserServiceRequestType(),
        ...
        try {
            payloadRequest = (CreateUserServiceRequestType) this.sch
                    .parsePayload(request.getRequestMessage().getRequestBody()
                            .getAny(), new CreateUserServiceRequestType());
            ...
            if (null == payload.getNames()
                    || ConstantADM.AW_USER_NAMES_MAX_LENGTH < payload
                            .getNames().length()) {
            ...
            DivisionGeografica divisionGeografica = this.divisionGeograficaJPAService
                    .findDivisionGeografica(request.getRequestMessage()
                            .getRequestHeader().getDestination()
                            .getServiceRegion(), this.em);
            ...
            AwRol role = this.awRolJPAService.findAwRolById(
                    Long.parseLong(payload.getRole()), this.em);
            ...
            AwUsuario currentUser = this.awUserJPAService.getUserByEmail(
                    payload.getCurrentUser(), this.em);
            ...
            AwUsuario newUser = this.awUserJPAService.getUserByEmail(
                    newUserEmail, this.em);
            ...
            if (!this.sch.successResponse(responseFront)) {
                logger.error(ErrorEnum.BROKER_ERROR, WebConsoleUtil
                        .generateString(responseFront.getResponseMessage()
    ...
    private CreateUserLDAPServiceRequestType buildCreateUserLdapRequest(
        ...
        createUserLdapRequestBody.setPhoneNumber(null != payloadRequest
                .getCreateUserRQ().getCellphone() ? payloadRequest
                .getCreateUserRQ().getCellphone() : null);
    ...
    private AwUsuario buildAwUser(CreateUserRQType payload,
        ...
        awUser.setApellidos(null != payload.getLastName() ? payload
                .getLastName() : null);
        awUser.setAwRol(role);
        awUser.setCelular(new BigDecimal(payload.getCellphone()));
        awUser.setEstado(Boolean.FALSE);
        awUser.setNombres(null != payload.getNames() ? payload.getNames()
                : null);
    ...
    private ResponseMessageObjectType createUserLdap(
        ...
        try {
            ...
            String registryResponse = serviceRegitryUtil.lookUpRegistryService(
                    ...
                    serviceInfo.getOperation(),
                    serviceInfo.getClassification(), serviceInfo.getRegion(),
            ...
            if (!Constant.COMMON_STRING_ZERO.equals(registryRsHeaders
                    .getStatus().getCode())) {
                logger.error(ErrorEnum.MIDDLEWARE_ERROR, registryRsHeaders
                        .getStatus().getDescription(), new MiddlewareException(
            ...
            LookupResponseType lookupResponse = registryRS
                    .getRegistryResponse().getBody().getLookupResponse();
            ...
            co.bdigital.admin.messaging.integration.ldap401.v100.RequestMessageObjectType integrationRq = WebConsoleUtil
                    .getLdap401RequestMessage(request.getRequestMessage()
            ...
            String endpoint = GenericRestClient.getUrlRestServicesEndPoint(
                    lookupResponse.getEndpoint().getProtocol(), lookupResponse
                            .getEndpoint().getHost(), lookupResponse
                            .getEndpoint().getPort(), lookupResponse
                            .getEndpoint().getContext());

            connectionTimeOut = Integer.parseInt(lookupResponse.getTimeout()
                    .getConnection());

            readTimeout = Integer.parseInt(lookupResponse.getTimeout()
                    .getRead());
            ...
            responseStatusType.setStatusDesc(integrationRs
                    .getIntegrationResponse().getHeader().getStatus()
                    .getDescription());
