...
public class UpdateUserServiceBean implements ServiceController {
    ...
    public ResponseMessageObjectType executeOperation(
        ...
        if (!this.sch.validateRequest(request.getRequestMessage()
                .getRequestBody().getAny(), new UpdateUserServiceRequestType(),
        ...
        try {
            payloadRequest = (UpdateUserServiceRequestType) this.sch
                    .parsePayload(request.getRequestMessage().getRequestBody()
                            .getAny(), new UpdateUserServiceRequestType());
            /*
            ...
            AwUsuario userToUpdate = this.awUserJPAService.getUserByEmail(
                    payload.getMail(), this.em);
    ...
    private AwUsuario buildAwUser(UpdateUserRQType payload,
        ...
        userToUpdate.setApellidos(null != payload.getLastName() ? payload
                .getLastName().trim() : null);
        userToUpdate.setAwRol(role);
        userToUpdate.setEstado(null != payload.getStatus()
                && Constant.COMMON_STRING_UNO.equalsIgnoreCase(payload
                        .getStatus()));
        userToUpdate.setNombres(null != payload.getNames() ? payload.getNames()
                .trim() : null);
