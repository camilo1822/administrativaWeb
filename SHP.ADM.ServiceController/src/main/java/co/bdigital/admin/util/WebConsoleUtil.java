...
public class WebConsoleUtil {
    ...
    public static UpdatePromotionRuleRequestType getUpdatePromotionRuleRequestType(
            ...
            String message, String maximumBudget) {
    ...
    /**
     * Utilidad que construye la petición para el componente de integración LDAP
     * 401.
     * 
     * @param messageId
     * @param channel
    ...
    public static String replaceAccentAndSpecialCharacter(String stringData) {
        if (null != stringData && !stringData.isEmpty()) {
            ...
            for (int i = ConstantADM.COMMON_ZERO; i < stringData
                    .length(); i++) {
                int position = ConstantADM.ORIGINAL.indexOf(arrayData[i]);
                if (position > -1) {
                    arrayData[i] = ConstantADM.TO_REPLACE.charAt(position);
                }
            }
